            ...
            infoList = infoList.filter(
                (info) => {
                    return (
                        ...
                        info.position.indexOf(this.state.keyword) > -1 || 
                        info.phone.indexOf(this.state.keyword) > -1 || 
                        info.address.indexOf(this.state.keyword) > -1 || 
                        info.email.toLowerCase().indexOf(this.state.keyword) > -1
                    );
                }
            );
            return (
                infoList.map((info, i) => (
                    <li key={i}>
                        <div className="dvbx">
                            <div className="item">
                                <div className="inbx">{info.name}</div>
                            </div>
                            <div className="item">
                                <div className="inbx">{info.position}</div>
                            </div>
                            <div className="item">
                                <div className="inbx">{info.phone}</div>
                            </div>
                            <div className="item">
                                <div className="inbx">{info.address}</div>
                            </div>
                            <div className="item">
                                <div className="inbx">{info.email}</div>
                            </div>
                        </div>
                    </li>
                ))
        ...
        return (
            <div className="finder_wrap">
                <div className="search_box">
                    <input type="text" placeholder="검색어를 입력하세요" autocomplete="off" value={this.state.keyword} onChange={this.handleChange} />
                </div>
                <div className="result_box">
                    ...
                    <div className="result_content">
                        <ul className="result_list">
                            {mapInfoList(this.state.infoList)}
